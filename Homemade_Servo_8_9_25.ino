...
int CMD_Max = 1024; // RAW ADC Value Max
...
int deadbandRange = 5; // +/- range for target value
...
void setup() {
  ...
  pinMode(PIN_ENA, OUTPUT);   // PWM will use analogWrite() later
...
void loop() {
  ...
  cmdValue = analogRead(PIN_CMD);
  fbValue = analogRead(PIN_FB);
  ...
  if (abs(error) > (cmdValue + deadband)) {
